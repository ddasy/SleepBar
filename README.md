...
- 🔁 **Timed Lock** — *“lock after **5 min** idle, for the next **2 hours**.”* Two numbers, one sentence: how long of no input triggers a lock, and how long the whole thing runs. SleepBar then auto-locks every time you've been idle that long, re-arming after each unlock, until the time's up. The "run for" duration is a fixed countdown — locking or unlocking never resets it. Prefer a clock time? Fill in **Until `07`:`30`** instead of a duration. The colon is fixed and the two halves are separate two-digit boxes: type two digits and the caret jumps to the minute by itself. Clicking into *any* box in this dialog selects what's in it, so every value is corrected by typing it again — never by backspacing from wherever the caret landed. The two ways of saying when to stop are either/or — typing in one clears the other. **No permissions required** (reads system idle time only, never intercepts input).
...
- 💬 **Report a problem / suggest a feature** — choose a type and describe your problem or suggestion in the app, optionally including system and app versions, then continue to a prefilled GitHub Issue in your browser. GitHub sign-in and confirmation are required; feedback is public, so leave out private information.
...
| Remembered preferences | `UserDefaults` (language / end action / custom duration / lock interval / window or stop time / last update check) |
