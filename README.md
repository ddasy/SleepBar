...
- 🔁 **Timed Lock** — *“lock after **5 min** idle, for the next **2 hours**.”* Two numbers, one sentence: how long of no input triggers a lock, and how long the whole thing runs. SleepBar then auto-locks every time you've been idle that long, re-arming after each unlock, until the time's up. The "run for" duration is a fixed countdown — locking or unlocking never resets it. Prefer a clock time? Fill in **Until 07:30** instead of a duration — the two ways of saying when to stop are either/or, and typing in one clears the other. **No permissions required** (reads system idle time only, never intercepts input).
