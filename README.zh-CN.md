...
| 偏好记忆 | `UserDefaults`(语言 / 到时间后动作 / 自定义时长) |
...
整个程序就是一个 `main.swift`(约 300 行 AppKit),没有任何第三方依赖。
...
一个免费开源的 macOS **菜单栏**小工具:**自动锁屏**、**定时休眠 / 睡眠定时器**、**屏幕关闭定时**、**保持唤醒 / 防止睡眠**、友好的 **caffeinate 图形界面**。原生 **Swift** 编写,可作为 Amphetamine、KeepingYouAwake、Caffeine 的轻量替代。

建议的 GitHub Topics:`macos` · `menubar` · `menu-bar` · `swift` · `appkit` · `caffeinate` · `sleep-timer` · `screen-lock` · `auto-lock` · `keep-awake` · `display-sleep` · `productivity` · `macos-app`
