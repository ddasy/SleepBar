...
- 🔒 **锁屏** —— 到点把屏幕锁到登录界面。
- 🖥️ **息屏** —— 「锁定并息屏」锁屏 + 关显示器更省电;另有独立的「息屏」让**内置屏亮度调到 0、外接屏经 DDC 关到真黑**(都不锁屏、不休眠显示器),屏幕全黑但 GPU 渲染不停摆,适合挂着跑渲染 / AI 任务过夜;回来动一下键鼠自动恢复(外接屏靠 DisplayPort 链路重训点亮)。
- 💤 **休眠** —— 到点直接让整机进入睡眠。
...
| **到时间后** | 息屏 | **内置屏**亮度调到 0、**外接屏**经 DDC 关屏(都真黑、**不锁屏**),保持唤醒、GPU 渲染不停摆;动一下键鼠**自动恢复**(外接屏靠链路重训点亮) |
| | 锁定屏幕 | 仅锁屏 |
...
| 息屏 · 外接屏 | `IOAVServiceWriteI2C` 发 DDC 电源关(VCP `D6=04`)关到真黑;回来时切一次分辨率再切回(`CGConfigureDisplayWithDisplayMode`)强制 DP 链路重训点亮 |
| 休眠 | `pmset sleepnow` |
