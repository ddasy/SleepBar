...
- 💬 **反馈问题 / 功能建议** —— 在应用内选择类型并填写标题、描述，再打开预填好的 GitHub Issue 页面。需登录 GitHub 并确认提交；内容公开，请勿填写隐私信息。
...
**Q: 它会偷偷联网吗?**
A: 自动联网只有一处:最多每 14 天一次,匿名 `GET` GitHub 公开的 Releases 接口比对版本号。不含任何身份标识、不做统计,也永远不会自动下载或安装。用 `run.sh` 起的开发版完全不检查。主动点击「前往 GitHub 提交」时，会通过浏览器把填写的标题和描述带到 GitHub；最终发布需在网页确认。
