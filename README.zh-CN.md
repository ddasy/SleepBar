...
- 🔁 **定时锁屏** —— 一句话设定:*「无操作 **5** 分钟即锁屏,持续 **2** 小时后自动停止」*。两个数字、一句话:多久没动键鼠就锁、整件事跑多久。设定后,每次空闲到时长就自动锁一次,解锁后重新开始计时,直到「持续」时间结束。「持续」是固定倒计时,**锁屏 / 解锁都不会重置它**。想按钟点收工?把「到 `07`:`30` 自动停止」填上代替「持续」即可。冒号是固定的,时和分是两个独立的两位数输入框:小时打满两位光标自动跳到分钟;发现填错了,再点一下那个框会**整体选中**,直接重打两位就覆盖,不用挪光标。两种写法**二选一**,填一个另一个会自动清空。**无需任何权限**(只读取系统空闲时间,绝不拦截输入)。
...
| **定时锁屏** | 定时锁屏… | 三行弹窗:**「无操作 `[N]` 分钟即锁屏」**,再从 **「持续 `[M]` 分钟后自动停止」** 和 **「到 `[时]`:`[分]` 自动停止」** 里**二选一**。时刻是固定冒号两侧的两个两位数框:小时打满两位自动跳到分钟,重新点某个框会整体选中、直接重打即可;填一个另一个会自动清空,填的时刻若今天已过则算明天。空闲到时长就自动锁一次,到点结束。**再点一次停止。** 填过的值会记住并预填 |
