...
- 🔒 **锁屏** —— 到点把屏幕锁到登录界面。
- 🖥️ **息屏** —— 「锁定并息屏」锁屏 + 关显示器更省电;另有独立的「息屏」让**内置屏亮度调到 0、外接屏经 DDC 关到真黑、键盘背光一并关闭、系统音量降到 1% 并静音**(都不锁屏、不休眠显示器),屏幕全黑、通知也不再出声,但 GPU 渲染不停摆,适合挂着跑渲染 / AI 任务过夜;回来动一下**鼠标或按一下键盘**就自动恢复(亮度、键盘背光、音量与静音状态一并还原,外接屏靠 DisplayPort 链路重训点亮)。
- 💤 **休眠** —— 到点直接让整机进入睡眠。
- ♾️ **强制不锁屏(永不)** —— 一键让屏幕一直常亮、绝不自动关闭,适合长任务 / 演示 / 看片。
- ☕ **不休眠** —— 「语言」下方的独立开关,勾上后整机就不会进入睡眠(显示器仍可正常关闭),与倒计时互不干扰、重启后依然记得;若某次「到时间后」动作本就是让电脑休眠,它会自动让路。
- 🔁 **定时锁屏** —— 一句话设定:*「无操作 **5** 分钟即锁屏,持续 **2** 小时后自动停止」*。两个数字、一句话:多久没动键鼠就锁、整件事跑多久。设定后,每次空闲到时长就自动锁一次,解锁后重新开始计时,直到「持续」时间结束。「持续」是固定倒计时,**锁屏 / 解锁都不会重置它**。想按钟点收工?把「到 `07`:`30` 自动停止」填上代替「持续」即可。冒号是固定的,时和分是两个独立的两位数输入框:小时打满两位光标自动跳到分钟。这个弹窗里的**每一个输入框**点进去都是**整体选中**状态,直接重打就覆盖,不用挪光标删旧值。两种写法**二选一**,填一个另一个会自动清空。**无需任何权限**(只读取系统空闲时间,绝不拦截输入)。
...
- 🔔 **自动检查更新** —— 每 14 天悄悄问一次 GitHub 有没有新版本;有的话菜单最上方多出一行,点开就是下载页。**不会自动下载、更不会自动安装**,更新与否你说了算;整个过程只发一个匿名请求,不携带任何个人信息。
- 💬 **反馈与建议** —— 在应用内选择类型并填写问题或建议，可勾选附带系统版本和 App 版本，再打开预填好的 GitHub Issue 页面。需登录 GitHub 并确认提交；内容公开，请勿填写隐私信息。
...
| 区块 | 选项 | 说明 |
|---|---|---|
| **更新** | 有新版本 X 可用 | **仅在**检测到新版本时才出现,点击打开下载页 |
| **屏幕关闭时间** | 立即 | **马上执行**「到时间后」选定的动作,无需倒计时 |
| | 5 / 10 / 15 / 30 分钟、1 小时 | 选中即开始倒计时;**再点一次取消** |
| | 自定义时长… | 弹窗输入任意分钟数,**记住并预填**上次的值 |
| | 永不 | 屏幕一直常亮,绝不自动关闭(菜单栏显示 `∞`) |
| **到时间后** | 息屏 | **内置屏**亮度调到 0、**外接屏**经 DDC 关屏、**键盘背光**关闭、**音量降到 1% 并静音**(都真黑真静音、**不锁屏**),保持唤醒、GPU 渲染不停摆;动鼠标或按键盘都能**自动恢复**(亮度、键盘背光、音量与静音状态,外接屏靠链路重训点亮) |
| | 锁定屏幕 | 仅锁屏 |
| | 锁定并息屏 | 锁屏 + 关闭(休眠)显示器,更省电 |
| | 锁定、息屏并休眠 | 锁屏 + 整机直接休眠 |
| | 锁定、息屏且不休眠 | 锁屏 + 关闭显示器,但系统保持唤醒不休眠 |
| **定时锁屏** | 定时锁屏… | 三行弹窗:**「无操作 `[N]` 分钟即锁屏」**,再从 **「持续 `[M]` 分钟后自动停止」** 和 **「到 `[时]`:`[分]` 自动停止」** 里**二选一**。时刻是固定冒号两侧的两个两位数框,小时打满两位自动跳到分钟;弹窗里每个输入框点进去都整体选中,直接重打就覆盖;填一个另一个会自动清空,填的时刻若今天已过则算明天。空闲到时长就自动锁一次,到点结束。**再点一次停止。** 填过的值会记住并预填 |
| **系统关屏时间** | *(当前档位)* → 1 分钟 … 3 小时 / 永不 | 显示系统「若无操作则关闭显示器」的当前档位,两下点击即可修改(会弹一次管理员密码框;写入的是你当前实际使用的电源配置) |
| | 提前 1 分钟自动息屏 | 在系统关屏时间前 1 分钟先执行 SleepBar 自家的无锁「息屏」,回来时屏幕黑着但不用输密码 |
| **语言** | 中文 · English · Español · العربية · Português (Brasil) · 日本語 · Deutsch | 整个菜单即时切换 |
| **不休眠** | 开关 | 勾上后整机不进入睡眠,直到你取消勾选;与倒计时独立,重启后仍记得 |
...
| 不休眠(常驻开关) | `caffeinate -is`,不带 `-t`,一直跑到你取消勾选或退出 App |
...
| 息屏 · 音频 | AppleScript 的 `volume settings`:先存下当前音量,降到 1% 并**静音**,回来时音量和静音状态一起还原。HDMI / 多数 USB 声卡 / AirPlay 这类 macOS 无法软件调音量的输出没有音量可存,靠静音生效 |
| 息屏 · 回归检测 | 全局鼠标事件监听 + 每秒读一次 `IOHIDSystem` 空闲时钟(按键同样会重置它),所以按键盘也能唤醒,且**无需辅助功能权限** |
| 系统关屏时间 | 读 `pmset -g`;写入用 `pmset -c\|-b displaysleep <分钟>`,经一次管理员密码授权,只写当前实际生效的电源配置 |
| 休眠 | `pmset sleepnow` |
...
| 检查更新 | 最多每 14 天匿名 `GET` 一次 `api.github.com/repos/ddasy/SleepBar/releases/latest` 比对版本号;用一次性定时器精确安排在到期时刻触发,期间零轮询 |
| 偏好记忆 | `UserDefaults`(语言 / 到时间后动作 / 自定义时长 / 锁屏间隔 / 持续时长或停止时刻 / 上次检查更新时间) |
...
整个程序就是一个 `main.swift`(约 1670 行 AppKit),没有任何第三方依赖。
...
**Q: 会偷偷让我的电脑一直不睡吗?**
A: 不会。只有你**主动选了时长或「永不」**才会保持常亮,只有你**主动勾了「不休眠」**才会阻止整机睡眠;启动默认是空闲状态,完全按你的系统设置走。

**Q: 重启后还在吗?**
A: 在。安装脚本把 SleepBar 注册为系统「登录项」,登录后自动出现在菜单栏。不需要的话,在菜单里取消勾选「开机自启」即可。

**Q: 我两种方式都装过,怎么办?**
A: 两份的后果是:两个登录项、Spotlight 里两个图标、菜单栏两个月亮,而更新只会落到其中一份上。两边都会自动收拾:`install.sh` 在安装前会把 `/Applications` 里那份**移到废纸篓**;正在运行的 App 如果发现另一份,也会弹窗问你要不要同样处理(先问再动,是移到废纸篓而不是删除,选「都保留」就不再打扰)。只有已安装的那份才会发出这个提示——从某个文件夹里直接跑的编译产物、或者 DMG 里还没拖出来的那份,绝不会去动你真正的安装。

**Q: 它会偷偷联网吗?**
A: 自动联网只有一处:最多每 14 天一次,匿名 `GET` GitHub 公开的 Releases 接口比对版本号。不含任何身份标识、不做统计,也永远不会自动下载或安装。用 `run.sh` 起的开发版完全不检查。主动点击「提交到 GitHub」时，会通过浏览器把填写的内容及勾选的版本信息带到 GitHub；最终发布需在网页确认。
